...
Java_ml_microsoft_onnxruntime_Env_initHandle(JNIEnv* env, jobject obj /* this */) {
  ...
  OrtEnv* ort_env;
  OrtCreateEnv(ORT_LOGGING_LEVEL_WARNING, "test", &ort_env);
...
Java_ml_microsoft_onnxruntime_Session_initHandle(JNIEnv* env, jobject obj /* this */,
  ...
  OrtCreateSession(ort_env, JStringtoStdString(env, j_model_path).c_str(), options, &session);
...
Java_ml_microsoft_onnxruntime_Session_run(JNIEnv* env, jobject obj /* this */,
  ...
  OrtRun(session, run_options, input_names.data(), input_values.data(), input_count, output_names.data(), output_count, output_values.data());
...
Java_ml_microsoft_onnxruntime_Value_createFloatTensorByData(JNIEnv* env, jobject obj /* this */,
                                                            jobject j_allocator_info, jfloatArray j_data, jlongArray j_shape) {
  ...
  const auto shape_len = env->GetArrayLength(j_shape);
  OrtValue* out;
  ORT_THROW_ON_ERROR(OrtCreateTensorWithDataAsOrtValue(allocator_info, data_ptr, data_len, shape_ptr, shape_len, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &out));
  jclass cls = env->FindClass("ml/microsoft/onnxruntime/Value");
  jobject j_value = env->AllocObject(cls);
...
Java_ml_microsoft_onnxruntime_AllocatorInfo_createCpu(JNIEnv* env, jobject obj /* this */,
  ...
  jint allocator_type_value = env->CallIntMethod(j_allocator_type,
                                                 env->GetMethodID(env->FindClass("ml/microsoft/onnxruntime/AllocatorInfo"), "ordinal", "()I"));
  jint mem_type_value = env->CallIntMethod(j_mem_type,
                                           env->GetMethodID(env->FindClass("ml/microsoft/onnxruntime/MemType"), "ordinal", "()I"));
  OrtAllocatorInfo* allocator_info;

  ORT_THROW_ON_ERROR(OrtCreateCpuAllocatorInfo(static_cast<OrtAllocatorType>(allocator_type_value), static_cast<OrtMemType>(mem_type_value), &allocator_info));
